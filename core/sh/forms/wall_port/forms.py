    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields['office'] = forms.ModelChoiceField(
            queryset=Office.objects.select_related(
                'loc__edifice__location__province',
                'dependency'
            ).all(),
        ...
        def label_from_instance(obj):
            return (
                f"OFICINA: {obj.office} / "
                f"PROVINCIA: {obj.loc.edifice.location.province.province} / "
                f"LOCALIDAD: {obj.loc.edifice.location.location} / "
                f"DEPENDENCIA: {obj.dependency.dependency}"
            )

        self.fields['office'].label_from_instance = label_from_instance
