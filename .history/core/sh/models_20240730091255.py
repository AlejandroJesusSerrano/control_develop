...
class Device(models.Model):
  dev_type = models.ForeignKey(Dev_Type, related_name = 'device_dev_type', verbose_name = 'Tipo de Dispositivo', on_delete = models.CASCADE )
  dev_model = models.ForeignKey(Dev_Model, related_name = 'device_dev_model', verbose_name = 'Modelo', on_delete = models.CASCADE)
  connection_type = models.ForeignKey(Connection_Type, related_name = 'device_connection_type', verbose_name='Tipo de Conexión', on_delete = models.CASCADE, null=True, blank=True)
  ip = models.CharField(max_length = 15, verbose_name = 'Direccion Ip', null = True, blank = True)
  dev_net_name = models.CharField(max_length = 15, verbose_name = 'Nombre en la Red', null = True, blank = True)
  dev_status = models.ForeignKey(Dev_Status, related_name = 'device_dev_status', verbose_name = 'Estado', on_delete = models.CASCADE)
  serial_number = models.CharField(max_length = 20, verbose_name = 'Número de Serie')
  wall_port = models.OneToOneField(Wall_Port, related_name = 'device_wall_port', verbose_name = 'Boca Pared',  on_delete = models.CASCADE, null = True, blank = True)
  switch_port = models.OneToOneField(Switch_Port, related_name = 'device_switch_port', verbose_name = 'Puerto del Switch', on_delete = models.CASCADE, blank=True, null=True)
  employee = models.ManyToManyField(Employee, related_name = 'device_employee', verbose_name = 'Empleado',  blank = True)
  date_creation = models.DateTimeField(auto_now = True, verbose_name = 'Fecha de Registro')
  date_updated = models.DateTimeField(auto_now_add = True, verbose_name = 'Última Modificación')

  def __str__(self):
    employees = ', '.join([employee.employee_last_name + ', ' + employee.employee_name for employee in self.employee.all()])
    return f'{self.dev_type} - {self.dev_model.brand} - {self.dev_model.dev_model} - {self.serial_number} - {self.ip} | USUARIO: {employees}'

  def toJSON(self):
    item = model_to_dict(self)
    item['dev_type'] = self.dev_type.dev_type,
    item['dev_model'] = self.dev_model.dev_model,
    item['connection_type'] = self.connection_type.connection_type if self.connection_type else 'No tiene tipo de conexión'
    item['ip'] = self.ip if self.ip else 'No tiene conexion a la red'
    item['dev_status'] = self.dev_status.dev_status
    item['wall_port'] = self.wall_port.wall_port if self.wall_port else 'No conecta a Boca de Pared'
    item['switch_in'] = 'Switch: '+str(self.switch_port.switch_in.brand)+' de '+str(self.switch_port.switch_in.ports_q)+' bocas, en el puerto '+str(self.switch_port.port_id)

    # item = model_to_dict(self)
    # item['dev_model'] = {
    #   'dev_model': self.dev_model.dev_model,
    #   'dev_type': self.dev_model.dev_type.dev_type,
    #   'brand': self.dev_model.brand.brand,
    #   'image': self.dev_model.image.url if self.dev_model.image else '/static/img/no_file.svg'
    #   }
    # item['connection_type'] = self.connection_type.connection_type if self.connection_type else 'No tiene tipo de conexión'
    # item['ip'] = self.ip if self.ip else 'No tiene conexion a la red'
    # item['dev_status'] = self.dev_status.dev_status
    # item['wall_port'] = {
    #   'wall_port': self.wall_port.wall_port,
    #   'office': self.wall_port.office.office,
    #   'switch_in' : {
    #     'brand': self.wall_port.switch_port_in.switch_in.brand.brand,
    #     'ports_q': self.wall_port.switch_port_in.switch_in.ports_q,
    #     'port': self.wall_port.switch_port_in.port_id
    #     }
    # } if self.wall_port else 'No conecta a Boca de Pared'
    # item['switch_in'] = 'Switch: '+str(self.switch_port.switch_in.brand)+'de '+str(self.switch_port.switch_in.ports_q)+' bocas, en el puerto '+str(self.switch_port.port_id) if self.switch_port else 'No conecta a switch intermedio'

    # if self.employee.exists():
    #   first_employee = self.employee.first()
    #   item['office'] = first_employee.office.office if first_employee.office else 'Sin oficina asignada'
    # else:
    #   item['office'] = 'Sin oficina asignada'

    # item['tech'] = self.tech.last_name if self.tech else 'Sin Técnico'

    # if self.employee.exists():
    #   employees = ', '.join([f'{e.employee_last_name}, {e.employee_name}' for e in self.employee.all()])
    #   item['employee'] = employees
    # else:
    #   item['employee'] = 'Sin empleado asignado'

    # item['connect'] = 'OFICINA: '+str(self.wall_port.office)+'BOCA: '+str(self.wall_port.wall_port) if self.wall_port else 'SWITCH: '+str(self.switch_port.switch.brand)+'PUERTO: '+str(self.switch_port.port_id)+'POSICION: '+str(self.switch_port.switch.switch_rack_pos)+'DEL RACK: '+str(self.switch_port.switch.rack.rack)
