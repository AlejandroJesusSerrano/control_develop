...
class OfficeLocForm(forms.ModelForm):
  ...
  class Meta:
    ...
    help_texts = {
      'wing': 'En caso de no tener un nombre de ala se recomiendqa poner el nombre de la calle hacia la que mira el ala.'
    }

  def __init__(self, *args, **kwargs):
    super(OfficeLocForm, self).__init__(*args, **kwargs)
    ...
    if self.instance.pk:
      ...
      self.fields['edifice'].queryset = Edifice.objects.filter(
        province = self.instance.edifice.location.province.province,
        location = self.instance.edifce.location.location
      )

    else:
      if 'province' in self.data and 'location' in self.data:
        try:
          province_id = int(self.data.get('province'))
          location_id = int(self.data.get('location'))
          self.fields['edifice'].queryset = Edifice.objects.filter(province_id=province_id, location_id=location_id)
