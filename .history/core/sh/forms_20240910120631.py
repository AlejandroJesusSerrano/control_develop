...
class OfficeForm(forms.ModelForm):
  ...
  class Meta:
    model = Office
    fields = [
      'dependency', 'loc', 'office', 'description'
      ]
    widget = {
      'dependency': forms.Select(attrs={'class': 'form-control select2'}),
      'loc': Select(attrs={'class': 'form-control select2'}),
      'office': TextInput(attrs={'placeholder': 'Ingrese un nombre identificatorio para la Oficina'}),
      'description': Textarea(attrs={'placeholder': 'Ingrese una descripción de la oficina'})
    }
    help_texts = {
  ...
  def __init_(self, *args, **kwargs):
    ...
    if self.instancce.pk:
      ...
      self.fields['loc'].queryset = Office_Loc.objects.filter(edifice = self.instance.edifice)
      ...
      if 'edifice'in self.data:
        try:
          edifice_id = int(self.data.get('edifice_id'))
  ...
  def clean(self):
    ...
    if Office.objects.filter(dependency=dependency, office__iexact=office).exists():
      self.add_error('office', f"Ya existe la oficina en la dependencia")
    cleaned_data = super().clean()
    return cleaned_data
