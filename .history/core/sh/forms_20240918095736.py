...
class ProvinceForm(forms.ModelForm):
  ...
  class Meta:
    ...
    help_texts = {
      'number_id': '* Por favor ingrese los dos digitos identificatorios de la provincia, Ej: 04 para "CÓRDOBA" 0 17 para "SALTA"'
    }
    ...
    def clean(self):
      ...
      if Province.objects.filter(number_id__iexact=number_id).exists():
        ...
        self.add_error('number_id', "Ya existe este registro de ID y Provincia")
        self.add_error('province', "Ya existe este registro de ID y Provincia")
      cleaned_data = super().clean()
      return cleaned_data
...
class EdificeForm(forms.ModelForm):
  ...
  def clean(self):
    ...
    if Edifice.objects.filter(location=location, edifice=edifice).exists():
      self.add_error('edifice', f"Ya existe un edificio con el nombre '{edifice}' en la localidad seleccionada")
    cleaned_data = super().clean()
    return cleaned_data
