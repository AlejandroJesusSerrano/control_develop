...
class LocationForm(forms.ModelForm):
  ...
  def clean(self):
    province_id = self.cleaned_data.get('province').id
    location_id = self.cleaned_data.get('location').id.upper()

    if Province.objects.filter(province_id=province_id, location_id=location_id).exists():
