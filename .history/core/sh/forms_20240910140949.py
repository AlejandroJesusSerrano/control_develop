  ...
  def __init__(self, *args, **kwargs):
    ...
    if self.instance.pk:
      ...
      if office.dependency:
        self.fields['dependency'].queryset = Dependency.objects.filter(location=office.dependency.location)

      if office.loc and office.loc.edifice:
        self.fields['edifice'].queryset = Edifice.objects.filter(location=office.loc.edifice)
        self.fields['loc'].queryset = Office_Loc.objects.filter(edifice=office.loc.edifice)
