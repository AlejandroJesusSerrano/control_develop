...
class DeviceForm(forms.ModelForm):
    ...
    ip = forms.CharField(
        widget=forms.TextInput(attrs={'class': 'form-control', 'placeholder': 'Si tuviera, ingrese la dirección ip del dispositivo'}),
        required=False
    )

    dev_net_name = forms.CharField(
        widget=forms.TextInput(attrs={'class': 'form-control', 'placeholder': 'Si tuviera, ingrese el nombre de registro en la red del dispositivo'}),
        required=False
    )

    dev_status = forms.ModelChoiceField(
        queryset=Dev_Status.objects.all(),
        widget=forms.Select(attrs={'class': 'form-control select2'})
    )

    serial_number = forms.CharField(
        widget=forms.TextInput(attrs={'class': 'form-control', 'placeholder': 'Ingrese el número de serie'})
    )
    ...
    wall_port = forms.ModelChoiceField(
    ...
    class Meta:
        model = Device
        fields = ['dev_type', 'brand', 'dev_model', 'connection_type', 'ip', 'dev_net_name', 'dev_status', 'serial_number', 'office', 'wall_port', 'switch_port', 'employee']
