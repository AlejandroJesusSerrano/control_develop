...
class ProvinceForm(forms.ModelForm):
  ...
  class Meta:
    ...
    help_texts = {
      'number_id': f'* Por favor ingrese los dos digitos identificatorios de la provincia, Ej: 04 para "CÓRDOBA" 0 17 para "SALTA"'
    }
    ...
    def clean(self):
      ...
      if Province.objects.filter(number_id__iexact=number_id).exists():
        self.add_error('number_id', f"El id de provincia ya se encuentra registrado")
      elif Province.object.filter(province__iexact=province).exists():
        self.add_error('province', f"La provincia ya se encuentra registrada")
      elif Province.objects.filter(province=province, number_id=number_id).exists:
        self.add_error('number_id', f"Ya existe este registro de ID y Provincia")
        self.add_error('province', f"Ya existe este registro de ID y Provincia")
      cleaned_data = super().clean()
      return cleaned_data
