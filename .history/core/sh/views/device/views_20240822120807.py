...
class DeviceUpdateView(UpdateView):
    ...
    def get_initial(self):
        ...
        # Preseleccionando valores
        initial['brand'] = device.dev_model.brand.id if device.dev_model and device.dev_model.brand else None
        initial['dev_type'] = device.dev_model.dev_type.id if device.dev_model and device.dev_model.dev_type else None
        initial['dev_model'] = device.dev_model.id if device.dev_model else None
        initial['wall_port'] = device.wall_port.id if device.wall_port else None
        initial['switch_port'] = device.switch_port.id if device.switch_port else None
        initial['employee'] = [emp.id for emp in device.employee.all()] if device.employee.exists() else None
    ...
    def get_context_data(self, **kwargs):
        ...
        # Configurando los querysets correctos para que los selects funcionen adecuadamente
        ...
        # Estableciendo los valores iniciales en el formulario para preselección
        context['form'].initial.update({
            'dev_model': device.dev_model.id if device.dev_model else None,
            'wall_port': device.wall_port.id if device.wall_port else None,
            'switch_port': device.switch_port.id if device.switch_port else None,
            'employee': [emp.id for emp in device.employee.all()] if device.employee.exists() else None,
        })
