...
class DeviceForm(forms.ModelForm):
    ...
    class Meta:
        model = Device
        fields = [
            'dev_type', 'brand', 'dev_model', 'connection_type', 'ip',
            'dev_net_name', 'dev_status', 'serial_number', 'office',
            'wall_port', 'switch_port', 'employee'
        ]
        widgets = {
            'dev_type': forms.Select(attrs={'class': 'form-control select2'}),
            'brand': forms.Select(attrs={'class': 'form-control select2'}),
            'dev_model': forms.Select(attrs={'class': 'form-control select2'}),
            'connection_type': forms.Select(attrs={'class': 'form-control select2'}),
            'ip': forms.TextInput(attrs={'class': 'form-control', 'placeholder': 'Si tuviera, ingrese la dirección ip del dispositivo'}),
            'dev_net_name': forms.TextInput(attrs={'class': 'form-control', 'placeholder': 'Si tuviera, ingrese el nombre de registro en la red del dispositivo'}),
            'dev_status': forms.Select(attrs={'class': 'form-control select2'}),
            'serial_number': forms.TextInput(attrs={'class': 'form-control', 'placeholder': 'Ingrese el número de serie'}),
    ...
    def __init__(self, *args, **kwargs):
        ...
        if 'dev_type' in self.data and 'brand' in self.data:
            try:
                dev_type_id = int(self.data.get('dev_type'))
                brand_id = int(self.data.get('brand'))
                self.fields['dev_model'].queryset = Dev_Model.objects.filter(dev_type_id=dev_type_id, brand_id=brand_id)
            except (ValueError, TypeError):
                pass  # queryset remains empty
...
#     employee = ModelMultipleChoiceField(
