...
class OfficeLocForm(forms.ModelForm):
  ...
  class Meta:
    ...
    help_text ={
      'wing': 'En caso de no tener un nombre de ala, se recomienda poner el nombre de la calla hacia la que mira la misma.'
    }
  ...
  def clean(self):
    ...
    if Office_Loc.objects.filter(edifice=edifice, floor=floor, wing=wing).exists():
      self.add_error('wing', f"Ya existe un ala con en el '{wing}' en el piso y edificio ingresado")
